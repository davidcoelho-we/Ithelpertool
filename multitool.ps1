...
function Show-Menu {
    ...
    Write-Host "6. Gerenciar Fast Startup (Ativar/Desativar)"
    Write-Host "0. Sair"
    Write-Host "======================="
}
...
function Lenovo-UpdateCV {
    ...
    if (!(Test-Path $suHelper)) {
        ...
        Write-Host "Download recomendado: https://github.com/davidcoelho-we/Ithelpertool/blob/main/VantageInstaller.zip" -ForegroundColor Yellow
        Read-Host "Pressione Enter para voltar ao menu..." | Out-Null
        return
    }
...
function Gerenciar-FastStartup {
    Clear-Host
    Write-Host "==== GERENCIAR FAST STARTUP ====" -ForegroundColor Cyan
    Write-Host "1. Ativar Fast Startup + Hibernação"
    Write-Host "2. Desativar Fast Startup + Hibernação"
    ...
    switch ($opt) {
        "1" {
            try {
                powercfg -h on
                Write-Host "`n✅ Fast Startup e Hibernação foram ATIVADOS com sucesso." -ForegroundColor Green
            } catch {
                Write-Host "`n❌ Erro ao ativar Fast Startup: $($_.Exception.Message)" -ForegroundColor Red
            }
        }
        "2" {
            try {
                powercfg -h off
                Write-Host "`n✅ Fast Startup e Hibernação foram DESATIVADOS com sucesso." -ForegroundColor Yellow
            } catch {
                Write-Host "`n❌ Erro ao desativar Fast Startup: $($_.Exception.Message)" -ForegroundColor Red
            }
        }
        default { return }
    }
